...
为仓库内 **Go API / Importer** 提供 OpenAI 兼容的 `POST /v1/embeddings`，实现见 [`app.py`](./app.py)。设计细节见 [`DESIGN.md`](./DESIGN.md)。
...
启动时会自动尝试加载 **仓库根目录** 与 **`python/embedding-service/.env`**（依赖 `python-dotenv`，已写入 `requirements.txt`）。未安装 `python-dotenv` 时，仍须在启动前自行 `export` / `set` 环境变量。
...
```powershell
cd d:\projects\agentic_search\ai-search-mvp\python\embedding-service
...
| 用途 | 变量（示例） | 谁读取 |
|------|----------------|--------|
| Go 调用的 HTTP 嵌入基址（可远端） | `EMBEDDING_API_BASE_URL`、`EMBEDDING_ENDPOINT`、`EMBEDDING_API_KEY`、`EMBEDDING_API_MODEL` | `cmd/api`、`cmd/importer` |
...
2. 在仓库根目录 `.env` 中设 **`EMBEDDING_SOURCE=self_hosted`**，并配置 `EMBEDDING_LOCAL_HTTP_*` / `EMBEDDING_LOCAL_API_KEY`（与 `internal/config/embedding.go` 一致）；或 `EMBEDDING_SOURCE=remote` 时用 `EMBEDDING_API_BASE_URL` / `EMBEDDING_ENDPOINT` 指向本机 URL。
...
- `EMBEDDING_SOURCE=remote`（或 `api`/`cloud`）：请求 `EMBEDDING_ENDPOINT` 或 `EMBEDDING_API_BASE_URL/v1/embeddings`，`Authorization: Bearer EMBEDDING_API_KEY`。
...
会加载根目录 `.env`，若存在 `python/embedding-service/.env` 则在其后覆盖。
